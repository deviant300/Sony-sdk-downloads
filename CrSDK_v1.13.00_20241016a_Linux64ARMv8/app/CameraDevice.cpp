...
void CameraDevice::execute_movie_rec()
{
    ...
    if( getContentsDataFlg == true ) {
        execute_movie_rec_and_get_contensdata();
    }
}
...
text CameraDevice::format_display_string_type(SDK::CrDisplayStringType type) {
    ...
    case SCRSDK::CrDisplayStringType_SubjectRecognitionAF_Display:
        ...
        break;
    case SCRSDK::CrDisplayStringType_Reserved11:
        ts << "reserved";
        break;
    case SCRSDK::CrDisplayStringType_Reserved12:
        ts << "reserved";
        break;
...
void CameraDevice::camera_button_function(bool multiFlg)
{
    ...
    SDK::CrDisplayStringListInfo* stringList;
    CrInt32u numOfList = 0;
...
void CameraDevice::get_remote_transfer_contentsdata()
{
    ...
    if( ret == SDK::CrError_None ) {
        if( outNums != 0 ) {
            ...
            release_contents_info(slotIndex);
            tout << "Get CapturedDateList Noting.\n";
            return;
        }
    }else{
        release_contents_info(slotIndex);
    ...
    if( (selected_index < 0) || (selected_index >= outNums) ) {
        release_contents_info(slotIndex);
        tout << "Input cancelled.\n";
        return;
    }
    ...
    while(1){
        ...
        memset(buff, 0, sizeof(buff));
        for(int i = 0;i < outNums;i++) {
            for(int j = 0;j < m_contentsInfoList[slotIndex][i].filesNum;j++) {
        ...
        const CrInt32u divisionSize = 0x5000000; // 50MB
        if( selected_index == 1 ) {
...
{   
    ...
    CrInt16u ipLen = hostIp.length() + 1;
...
void CameraDevice::execute_movie_rec_and_get_contensdata()
{
    m_getContentsDataStartFlg = true;
    std::unique_lock<std::mutex> lock(m_getContentsDataMtx);
    m_getContentsDataMovieCv.wait(lock);
    ...
    for(int i = 0;i < contentsInfoListNum;i++) {
    ...
    const CrInt32u divisionSize = 0x5000000; // 50MB
    for(int i = 0;i < targetContentsInfo.filesNum;i++) {
...
void CameraDevice::show_contents_data_detail(SCRSDK::CrContentsInfo& contentsInfo, SCRSDK::CrContentsFile& contentsFile)
{
    ...
    tout << buff;
    for(int i =0;i < contentsInfo.shotMarkNum;i++) {
    ...
    tout << buff;
    snprintf(buff, sizeof(buff), "fileSize = %llu\n", contentsFile.fileSize);
    tout << buff;
    tout << "umid = 0x";
